...
if(!function_exists('tech888f_reg_metabox')){
    function tech888f_reg_metabox($id = null, $title = null, $callback = null, $screen = null, $context = null, $priority = null, $callback_args = null){
...
if(!function_exists('tech888f_reg_taxonomy'))
{
    function tech888f_reg_taxonomy($taxonomy, $object_type, $args )
    {
        register_taxonomy($taxonomy, $object_type, $args );
    }
}
...
if(!function_exists('tech888f_shortcode_param'))
{
    function tech888f_shortcode_param( $name, $form_field_callback, $script_url = null ){
        add_shortcode_param( $name, $form_field_callback, $script_url = null );
    }
}
if(!function_exists('tech888f_instagram_api_curl_connect')){
    function tech888f_instagram_api_curl_connect( $api_url ){
        $content = file_get_contents($api_url);
        return json_decode( $content ); // decode and return
    }
}
if(!function_exists('tech888f_scrape_instagram'))
{
function tech888f_scrape_instagram($username, $slice = 9 , $token = '',$size_index= 0) {
    ...
    if($username) {
        ...
        return new WP_Error('site_down', __('Unable to communicate with Instagram.', 't888-core'));
        if ( 200 != wp_remote_retrieve_response_code( $remote ) )
        return new WP_Error('invalid_response', __('Instagram did not return a 200.', 't888-core'));
        $shards = explode('window._sharedData = ', $remote['body']);
        $insta_json = explode(';</script>', $shards[1]);
        $insta_array = json_decode($insta_json[0], TRUE);
        if (!$insta_array)
        return new WP_Error('bad_json', __('Instagram has returned invalid data.', 't888-core'));
        if(!empty($token)){
            ...
            while ($api !== NULL && $i <= $max_page) {                
                $data = tech888f_instagram_api_curl_connect($api);
...
if(!function_exists('tech888f_images_only'))
{
    function tech888f_images_only($media_item) {
        if ($media_item['type'] == 'image')
        return true;
        return false;
    }
}
if(!function_exists('tech888f_get_current_url'))
{
    function tech888f_get_current_url() {
        $url = $_SERVER["HTTP_HOST"] . $_SERVER["REQUEST_URI"];
        return $url;
    }
}
if(!function_exists('tech888f_get_price_arange')){
    function tech888f_get_price_arange(){
...
if(!function_exists('tech888f_load_lib')){
    function tech888f_load_lib($folder)
    {
        //Auto load widget
        $files=glob(get_template_directory()."/"."t888core/".$folder."/*.php");
...
function tech888f_get_image_by_url($image_src,$size,$class=''){
    global $wpdb;
    $width = $height = '';
    if(is_array($size)){
        $width = $size[0];
...
if (!function_exists('tech888f_add_inline_style')) {
    function tech888f_add_inline_style($style) {
...
if(!function_exists('tech888f_fix_decodeb64')){
    function tech888f_fix_decodeb64( $data , $strict = FALSE ){
